...
import { Route } from './neighborhood'
...
async function createVaultFixture(files: Record<string, string>) {
  const root = await mkdtemp(path.join(os.tmpdir(), 'nabu-vault-neighborhood-route-'))
  tempRoots.push(root)
...
describe('GET /api/vault/notes/neighborhood', () => {
  it('returns 401 when request is unauthenticated', async () => {
    ...
    const response = await handler({
      request: new Request('http://localhost:3000/api/vault/notes/neighborhood?path=projects/nabu/roadmap.md'),
    })
  ...
  it('returns neighborhood payload when request is authenticated', async () => {
    await createVaultFixture({
      'projects/nabu/roadmap.md': '[[projects/nabu/vision.md]]',
      'projects/nabu/vision.md': '# Vision',
      'projects/nabu/todo.md': '[[projects/nabu/roadmap.md]]',
    })
  ...
  it('returns 400 for invalid path input when request is authenticated', async () => {
    ...
    const session = createSessionToken()
    const response = await handler({
      request: new Request('http://localhost:3000/api/vault/notes/neighborhood?path=../secrets.md', {
        headers: {
          cookie: `${AUTH_COOKIE_NAME}=${encodeURIComponent(session)}`,
        },
      }),
